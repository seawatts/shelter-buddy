...
export const finishWalkAction = authenticatedAction
  ...
  .handler(async ({ ctx, input }) => {
    ...
    const roomId = walk.animal.kennelOccupants[0]?.kennel.room.id;

    if (!roomId) {
      throw new Error("Room not found for animal");
    }

    revalidatePath(`/shelters/${walk.shelterId}/rooms/${roomId}/kennels`);
    redirect(`/shelters/${walk.shelterId}/rooms/${roomId}/kennels`);
    return { success: true };
  });
...
export const endWalkAction = authenticatedAction
  ...
  .handler(async ({ input }) => {
    ...
    revalidatePath(`/shelters/${walk.shelterId}/animals/${walk.animalId}`);
    redirect(
      `/shelters/${walk.shelterId}/animals/${walk.animalId}/walks/${walk.id}/finished`,
    );
  });
