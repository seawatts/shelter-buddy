...
import {Activity, Equipment, IntakeForm, TestResponse, DifficultyLevel, Gender} from "./types"
...
export class BamlAsyncClient {
  ...
  async SimpleTest(
    ...
    try {
      ...
      return raw.parsed() as TestResponse
    } catch (error: any) {
      const bamlError = createBamlValidationError(error);
      if (bamlError instanceof BamlValidationError) {
        throw bamlError;
...
class BamlStreamClient {
  ...
  SimpleTest(
      input: string,
      __baml_options__?: { tb?: TypeBuilder, clientRegistry?: ClientRegistry }
  ): BamlStream<RecursivePartialNull<TestResponse>, TestResponse> {
    try {
      const raw = this.runtime.streamFunction(
        "SimpleTest",
        {
      ...
      return new BamlStream<RecursivePartialNull<TestResponse>, TestResponse>(
        raw,
        (a): a is RecursivePartialNull<TestResponse> => a,
        (a): a is TestResponse => a,
        this.ctx_manager.cloneContext(),
        __baml_options__?.tb?.__tb(),
      )
    } catch (error) {
